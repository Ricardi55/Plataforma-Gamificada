...
// Tabla recomendada para la experimentacion. Debe existir en Supabase.
const SUPABASE_TABLE = process.env.SUPABASE_TABLE || "mutapi_history";
...
function saveAttempt(attempt) {
  ...
  saveHistoryEvent("attempt", record).catch(error => {
    console.warn("No se pudo guardar intento en Supabase", error.message);
  });
}
...
function saveSession(session) {
  ...
  fs.writeFileSync(RESPONSES_FILE, JSON.stringify(db, null, 2));
  saveHistoryEvent("session", record).catch(error => {
    console.warn("No se pudo guardar sesion en Supabase", error.message);
  });
}

async function saveHistoryEvent(eventType, payload) {
  if (!hasSupabaseConfig()) return;

  const body = buildSupabaseHistoryRow(eventType, payload);
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${SUPABASE_TABLE}?on_conflict=event_type,external_id`, {
    method: "POST",
...
function buildSupabaseHistoryRow(eventType, payload) {
  const exercise = findExercise(payload.type, payload.exerciseId);
  const isAttempt = eventType === "attempt";
  const externalId = isAttempt
  ...
  return {
    ...
    level: payload.level != null ? Number(payload.level) : null,
    level_type: payload.type || null,
    exercise_id: isAttempt ? (payload.exerciseId || null) : null,
    session_id: payload.sessionId || null,
    correct: isAttempt && typeof payload.correct === "boolean" ? payload.correct : null,
    score: isAttempt && payload.score != null ? Math.round(Number(payload.score)) : null,
    duration_seconds: payload.durationSeconds != null ? Number(payload.durationSeconds) : null,
    remaining_seconds: payload.remainingSeconds != null ? Number(payload.remainingSeconds) : null,
    used_hint: typeof payload.usedHint === "boolean" ? payload.usedHint : null,
    used_hint_type: payload.usedHintType || null,
    had_wrong_attempt: typeof payload.hadWrongAttempt === "boolean" ? payload.hadWrongAttempt : null,
    status: payload.status || (isAttempt ? (payload.correct ? "correct" : "incorrect") : null),
    payload: {
      event_type: eventType,
      saved_at: new Date().toISOString(),
      question: isAttempt ? getExerciseQuestion(exercise) : null,
      user_answer: isAttempt ? stringifyAnswer(payload.answer) : null,
...
function findExercise(type, exerciseId) {
  if (!type || !exerciseId || !exercises[type]) return null;
  return exercises[type].find(item => item.id === exerciseId) || null;
}

function getExerciseQuestion(exercise) {
  if (!exercise) return null;
  return exercise.question || exercise.prompt || exercise.title || null;
}
...
async function readHistoryEvents() {
  if (!hasSupabaseConfig()) return readResponsesStore();
...
function supabaseHeaders(mode = "read") {
  const headers = {
    "Content-Type": "application/json",
    apikey: SUPABASE_SERVICE_ROLE_KEY,
    Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`
  };
  if (mode === "upsert") headers.Prefer = "return=minimal,resolution=merge-duplicates";
  return headers;
}
...
function routeApi(req, res, url) {
  ...
  if (req.method === "GET" && url.pathname === "/api/responses") {
    return readHistoryEvents()
      ...
      .catch(() => sendJson(res, 200, readResponsesStore()));
  }
...
http.createServer((req, res) => {
  ...
  console.log(hasSupabaseConfig()
    ? `Historico conectado a Supabase: ${SUPABASE_TABLE}`
    : "Supabase no configurado. Se usara data/responses.json como respaldo local.");
});
